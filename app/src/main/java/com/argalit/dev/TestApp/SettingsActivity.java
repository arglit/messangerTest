...
public class SettingsActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        getUserDataReference = FirebaseDatabase.getInstance().getReference().child("Users").child(online_user_id);//ссылка на корень базы данных firebase
        getUserDataReference.keepSynced(true);

        storeProfileImagestorageRef = FirebaseStorage.getInstance().getReference().child("Profile_Images");// ссылка папки а storage

        thumbImageRef = FirebaseStorage.getInstance().getReference().child("Thumb_Images");// ссылка на сжатые изображения
        ...
        getUserDataReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) // получение данных пользователя из базы
            {
                ...
                if(!image.equals("default_profile"))
                {
                    ...
                    Picasso.get().load(image)
                            .networkPolicy(NetworkPolicy.OFFLINE).placeholder(R.drawable.default_profile).into(settingsDisplayProfileImage, new Callback() {
                        @Override
        ...
        {
            //отправка пользователя в галерею
            @Override
    ...
    //обрезка изображения с помощью ArthurHub
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data)
    {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE)
        {
            ...
            if (resultCode == RESULT_OK)
            {
                ...
                //сохранение изображения по индификатору
                ...
                filePath.putFile(resultUri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>()//Загрузка исходных изображений в профиль
                {
                    @Override
                    public void onComplete(@NonNull final Task<UploadTask.TaskSnapshot> task)
                    {
                        if (task.isSuccessful())
                        {
                            ...
                            final String downloadUri = task.getResult().getDownloadUrl().toString();//адрес изображения в Firebase
                        ...
                        {
                            Toast.makeText(SettingsActivity.this, "Error occurred, while uploading your profile pic ", Toast.LENGTH_SHORT).show();
